describe('GET /api/agents/[id]/summary (BDD)', () => {
  ...
  it('Given backend returns JSON, Then proxy returns JSON', async () => {
    ...
    (global.fetch as any).mockResolvedValueOnce({ ok: true, headers: new Map([['content-type','application/json']]), json: async () => ({ ok: true }) });
